...
Page({
  ...
  nextsongs: async function (event){

    let person_datas =  await request('/personal_fm');
    let index=2;
    let indexs  = this.data.indexline+3
    let datalist = this.data.person_data.push(person_datas)
    this.setData({
      person_data:datalist,
      indexline: indexs
    })
